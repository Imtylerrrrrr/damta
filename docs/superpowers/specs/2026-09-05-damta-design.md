...
- **머금기(hold)**: 빨기가 끝나면(가득 참/입 뗌) `exhaleLock`. 입이 중립(pucker<0.3·funnel<0.2·jawOpen<0.15)인 상태로 0.3s 지나야 풀림. 사용자 피드백(9/5): '오' 유지 시 빨기→내뿜기 즉시 전환 루프가 어색 → 잠금 도입.
- **내뿜기(exhale)**: 폐>0.1 + 빨고 있지 않음 + 잠금 해제 + (jawOpen>0.25 또는 O 입모양) → 0.22s마다 도넛 링 방출, 폐 감소.
